...
async function downloadBest50Image(report: ScoreImportReport) {
  ...
  const drawSection = (bucket: 'b35' | 'b15', startY: number) => {
    ...
    entries.forEach((entry, index) => {
      ...
      context.fillText(
        clippedText(context, entry.title, textWidth),
        textX,
        y + 31,
      );
      ...
      context.fillText(
        `${Number(entry.achievement).toFixed(4)}%`,
        textX,
        y + 58,
      );
      context.fillStyle = '#94a3b8';
...
export default function ScoreImportReportPage() {
  ...
  useEffect(() => {
    apiFetch(`/v1/imports/scores/${params.importId}`)
      .then(async (response) => {
        const payload = (await response.json()) as ScoreImportReport & {
          detail?: string;
        };
        if (!response.ok)
      ...
      .catch((reason: unknown) => {
        setError(
          reason instanceof Error ? reason.message : '无法读取成绩导入报告',
        );
      });
  }, [params.importId]);
    ...
    return (
      <main className="grid min-h-screen place-items-center bg-background p-6 text-foreground">
        <div className="max-w-lg rounded-3xl border border-rose-300/20 bg-card p-8 text-center">
          ...
          <a
            href="/"
            className="mt-6 inline-flex items-center gap-2 text-sm text-cyan-200"
          >
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-background px-4 py-6 text-foreground sm:px-8 sm:py-8">
      <div className="mx-auto max-w-6xl">
        <a
          href="/"
          className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-cyan-200"
        >
          <ArrowLeft className="size-4" /> 返回首页
        </a>

        <section className="mt-6 min-w-0 overflow-hidden rounded-[2rem] border border-white/10 bg-card/75 p-4 sm:p-9">
          <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0">
              <Badge
                ...
                className={
                  succeeded
                    ? 'border-lime-300/30 bg-lime-300/8 text-lime-200'
                    : 'border-amber-300/30 bg-amber-300/8 text-amber-200'
                }
              >
                {succeeded ? 'IMPORT COMPLETE' : 'REVIEW NEEDED'}
              </Badge>
              <h1 className="mt-4 break-words font-display text-2xl font-black sm:text-5xl">
                {succeeded ? '完整成绩读取成功' : '成绩已读取，还有项目待确认'}
              </h1>
              <p className="mt-3 text-sm leading-6 text-muted-foreground">
                抓到 {report.suppliedCount} 条记录，整理为 {effectiveCount}{' '}
                张有效谱面。
                {report.correctedTypeCount > 0 &&
                  ` 已自动纠正 ${report.correctedTypeCount} 条 DX/STD 类型。`}
              </p>
            </div>
            <div
              className={`grid size-24 shrink-0 place-items-center rounded-full border ${succeeded ? 'border-lime-300/25 bg-lime-300/8' : 'border-amber-300/25 bg-amber-300/8'}`}
            >
              {succeeded ? (
                <CheckCircle2 className="size-12 text-lime-300" />
              ) : (
                <TriangleAlert className="size-12 text-amber-300" />
              )}
            </div>
          </div>
        ...
        <section className="mt-6 min-w-0 rounded-3xl border border-lime-300/15 bg-lime-300/5 p-4 sm:flex sm:items-center sm:justify-between sm:gap-6 sm:p-6">
          <div className="min-w-0">
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-lime-200">
              {report.b50Source === 'official_dxnet'
                ? 'Official DX NET B50'
                : 'Calculated B50 fallback'}
            </p>
            ...
            {report.b50Generated && (
              <p className="mt-3 flex flex-wrap items-baseline gap-x-2 gap-y-1 font-mono text-xl font-black text-white sm:text-2xl">
                <span>{report.totalRating}</span>
            ...
            {report.b50Source === 'calculated_fallback' && (
              <p className="mt-2 text-xs font-semibold text-amber-200">
                官网榜单未成功读取；请先检查这份 B50，之后可手动修改或上传 B50
                图片校正。
              </p>
            )}
          </div>
        ...
        {report.best50Entries.length === 50 && (
          <section className="mt-6 min-w-0 rounded-3xl border border-white/8 bg-card/55 p-4 sm:p-6">
            <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.18em] text-cyan-200">
                  B35 / B15 verification
                </p>
                <h2 className="mt-2 font-display text-2xl font-bold">
                  检查当前 B50
                </h2>
            ...
            <div className="mt-6 grid gap-6 lg:grid-cols-2">
              {(['b35', 'b15'] as const).map((bucket) => (
                <div key={bucket}>
                  <h3
                    className={`text-sm font-black ${bucket === 'b35' ? 'text-cyan-200' : 'text-lime-200'}`}
                  >
                  ...
                  <div className="mt-3 divide-y divide-white/7 overflow-hidden rounded-2xl border border-white/8 bg-slate-950/25">
                    {report.best50Entries
                      .filter((entry) => entry.bucket === bucket)
                      .map((entry) => (
                        <div
                          key={`${bucket}-${entry.position}`}
                          className="flex min-w-0 items-center gap-2 px-2 py-2.5 text-sm sm:gap-3 sm:px-3"
                        >
                          <span className="w-7 shrink-0 font-mono text-xs font-bold text-muted-foreground">
                            #{entry.position}
                          </span>
                          <div className="relative size-10 shrink-0 overflow-hidden rounded-lg bg-white/5 sm:size-12">
                            {entry.coverUrl ? (
                              <Image
                                src={entry.coverUrl}
                                alt=""
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
          <section className="min-w-0 rounded-3xl border border-white/8 bg-card/55 p-4 sm:p-6">
          ...
          <section className="rounded-3xl border border-white/8 bg-card/55 p-6">
            ...
            <div className="mt-5 divide-y divide-white/7">
              {report.sampleScores.map((score, index) => (
                <div
                  key={`${score.title}-${score.difficulty}-${index}`}
                  className="flex min-w-0 flex-wrap items-center gap-x-3 gap-y-1 py-3 sm:flex-nowrap sm:gap-4"
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      ...
                      {difficultyLabels[score.difficulty] ?? score.difficulty}
                    </p>
                  </div>
                  {score.fullCombo && (
                  ...
                  <span className="ml-auto font-mono text-sm font-bold text-cyan-100">
        ...
        {report.issues.length > 0 && (
          <section className="mt-6 rounded-3xl border border-amber-300/20 bg-amber-300/5 p-6">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              先显示前 20 项，共 {report.issues.length} 项。
            </p>
            <div className="mt-4 grid gap-2 sm:grid-cols-2">
              {report.issues.slice(0, 20).map((issue) => (
                <div
                  key={issue.sourceIndex}
                  className="min-w-0 rounded-xl border border-amber-200/10 bg-black/10 px-4 py-3 text-sm"
                >
                  <p className="break-words font-semibold">{issue.title}</p>
                  <p className="mt-1 break-words text-xs text-amber-100/65">
                    {issue.chartType.toUpperCase()} ·{' '}
                    {difficultyLabels[issue.difficulty] ?? issue.difficulty} ·{' '}
                    {issue.issueCode}
                  </p>
                </div>
              ))}
        ...
        <p className="mt-6 text-center text-xs text-muted-foreground">
          官网 Rating Target
          成功读取时直接采用；只有官网榜单不可用时才显示本地临时重算结果。
        </p>
      </div>
    </main>
  );
